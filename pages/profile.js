...
const Profile = () => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        const { data } = await axios.get('https://backend-app-training.onrender.com/api/users/profile', {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.put('https://backend-app-training.onrender.com/api/users/profile', {
  ...
  const handleDeleteAccount = async () => {
    ...
    setTimeout(async () => {
      try {
        const token = localStorage.getItem('token');
        await axios.delete('https://backend-app-training.onrender.com/api/users/profile', {
          headers: { Authorization: `Bearer ${token}` },
        });
